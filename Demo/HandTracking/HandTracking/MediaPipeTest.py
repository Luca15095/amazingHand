...
def process_img(hand_proc, image):
    ...
    if results.multi_hand_landmarks:
      ...
      for index,handedness_classif in enumerate(results.multi_handedness):
          if handedness_classif.classification[0].score>0.8: #let's considere only one hand
              ...
              if handedness_classif.classification[0].label=='Left':
                  R=np.array([unit_x,-unit_y,unit_z]).reshape((3,3)) #-y because of mirror?
            #Tip 1-4 werden in Lokalen Referenzsystem ausgedrückt
              ...
              #print("das ist tip1 mit x_y_z {tip1}")
              # scale=0.01
...
def stand_alone_main():
    # cv2.VideoCapture(0) greift auf die erste verfügbare Kamera zu
    ...
    with mp_hands.Hands(
        ...
        while cap.isOpened():
            ...
            # Spiegeln des Frames, um eine 'Selfie'-Ansicht zu erhalten
            frame = cv2.flip(frame, 1)

            # --- Verarbeitung ---
            frame, r_res, l_res = process_img(hands, frame)

            # --- Ergebnisse Prüfen und Ausgeben (ersetzt node.send_output) ---
            '''if r_res is not None:
                # Die Ausgabe an Dora wären die Werte in 'r_res'
                # Normalerweise ein flaches Array von floats: [x0, y0, z0, x1, y1, z1, ..., x20, y20, z20]
                print(f"Inhalt von r_res: {r_res}")
                
                #print(f"✅ Rechte Hand erkannt! Datenstrom (erster Punkt): X={r_res[0]:.4f}, Y={r_res[1]:.4f}, Z={r_res[2]:.4f}")
                # 
            if l_res is not None:
                # Die Ausgabe an Dora wären die Werte in 'l_res'
                print(f"Inhalt von l_res: {tip1_x}")
                #print(f"✅ Linke Hand erkannt! Datenstrom (erster Punkt): X={l_res[0]:.4f}, Y={l_res[1]:.4f}, Z={l_res[2]:.4f}")
            
            if r_res is None and l_res is None:
                print("❌ Keine Hand erkannt.")'''

            # --- Visualisierung ---
            cv2.imshow('MediaPipe Hands Stand-Alone Test', frame)

            # Beenden bei Drücken von 'q'
