...
# TRICK: Wir gaukeln MuJoCo vor, dass wir mjpython sind.
# Da mjpython in Conda oft nur ein Wrapper-Skript ist, zeigt sys.executable
# auf das normale Python. Wir korrigieren das manuell, um den Check zu bestehen.
if sys.platform == 'darwin':
    # Der Pfad muss 'mjpython' enthalten, damit der Check in viewer.py durchgeht.
    # Es ist egal, ob der Pfad zu 100% stimmt, solange das Keyword drin ist.
   sys.executable = '/opt/homebrew/Caskroom/miniconda/base/envs/amazingHand/bin/mjpython'
import mink
...
class Client:
    ...
    def __init__(self, mode='pos'):
        ...
        #Baut die Hand in der Simulationsumgebung auf
        self.model = mujoco.MjModel.from_xml_path(
            f"{ROOT_PATH}/AHSimulation/AH_Left/mjcf/scene.xml"
        )
        # self.data=mujoco.MjData(self.model)

        #Stellt die Aktuelle Position der Hand innerhalb der Simulation da
        ...
        if mode=='pos': #we control the position
                ...
                if event_type == "INPUT":
                    ...
                    if event_id == "tick":
                        ...
                        if len(self.metadata) > 0:
                            ...
                            # # 2. Sauberer Debug-Print
                            # print(f"\n--- [DEBUG] Output: mj_l_joints_pos ---")
                            # print(f"Timestamp: {self.metadata.get('timestamp', 'N/A')}")
                            # # Wir runden die Werte auf 3 Nachkommastellen für die Lesbarkeit
                            # readable_pos = np.round(self.motor_pos, 3).tolist()
                            # print(f"Motor Positions (8 joints): {readable_pos}")
                            # print(f"Finger Mapping: f1:{readable_pos[0:2]}, f2:{readable_pos[2:4]}, f3:{readable_pos[4:6]}, f4:{readable_pos[6:8]}")
                            # print(f"----------------------------------------\n")
